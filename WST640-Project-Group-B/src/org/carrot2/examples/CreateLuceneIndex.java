...
public class CreateLuceneIndex
{
        ...
        throws Exception
    {
        ...
        for (Document d : SampleDocumentData.DOCUMENTS_DATA_MINING)
        {
            ...
            System.out.println(d.getContentUrl());
        ...
        for (int i = 0; i<11; i++){
        	final org.apache.lucene.document.Document doc = new org.apache.lucene.document.Document();
        	doc.add(new TextField("fullContent", "hej", Store.NO));
            doc.add(new TextField("title", "hej title", Store.YES));
            doc.add(new TextField("snippet", "nksdns", Store.YES));
            doc.add(new StringField("url", "jjjll", Store.YES));
